    ...
    if (error) {
        return (
            <div className="p-6 flex flex-col items-center justify-center min-h-[400px] text-center">
                ...
                <h2 className="text-lg font-semibold mb-2">Failed to load hotels</h2>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-6 space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
                        <Hotel className="w-6 h-6 text-cheese-600" /> Hotels
                    </h1>
                    <p className="text-sm text-muted-foreground mt-1">
                        {isLoading ? "…" : `${filtered.length} hotel establishments`}
                    </p>
                </div>
                            ...
                            <Card className="border border-border shadow-sm hover:shadow-md transition-all cursor-pointer" onClick={() => navigate(`/cheese/hotel-reservations?hotel=${encodeURIComponent(hotel.name)}`)}>
                                <CardContent className="p-5">
                                    <div className="flex items-start justify-between mb-3">
                                        ...
                                        <Badge className="bg-indigo-500/15 text-indigo-600">Hotel</Badge>
                                    </div>
                                    <div className="space-y-1.5 text-xs text-muted-foreground">
                                        <div className="flex items-center gap-2">
                                            <BedDouble className="w-3.5 h-3.5" />
                                            {hotel.experience_count ?? 0} room type{(hotel.experience_count ?? 0) !== 1 ? "s" : ""}
                                        </div>
                                        ...
                                        {hotel.administrator_contact && (
                                            <p className="truncate">Contact: {hotel.administrator_contact}</p>
                                        )}
                                    </div>
                                </CardContent>
                            </Card>
            ...
            {!isLoading && filtered.length === 0 && (
                <div className="text-center py-16">
                    ...
                    <h3 className="text-lg font-semibold text-muted-foreground">No hotels found</h3>
                    <p className="text-sm text-muted-foreground mt-1">
                        Mark an establishment as "Is Hotel" to see it here.
                    </p>
                </div>
            )}
        </motion.div>
