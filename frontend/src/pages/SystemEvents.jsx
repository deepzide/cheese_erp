    ...
    if (error) {
        return (
            <div className="p-6 flex flex-col items-center justify-center min-h-[400px] text-center">
                <AlertCircle className="w-12 h-12 text-red-400 mb-4" /><h2 className="text-lg font-semibold mb-2">{t("systemEvents.loadFailed", "Failed to load events")}</h2>
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-6 space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-foreground flex items-center gap-2"><Activity className="w-6 h-6 text-cheese-600" /> {t("systemEvents.title", "System Events")}</h1>
                    <p className="text-sm text-muted-foreground mt-1">{isLoading ? "..." : `${filtered.length} ${t("systemEvents.events", "events")}`} — {t("systemEvents.auditLog", "Audit Log")}</p>
                </div>
                <div className="flex gap-2">
                    <div className="relative"><Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" /><Input placeholder={t("systemEvents.search", "Search...")} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9 w-56 h-9" /></div>
                    <Select value={filterType} onValueChange={setFilterType}>
                        <SelectTrigger className="w-44 h-9"><Filter className="w-3 h-3 mr-1" /><SelectValue /></SelectTrigger>
                        <SelectContent>
                            <SelectItem value="all">{t("systemEvents.allTypes", "All Types")}</SelectItem>
                            ...
                            <motion.div key={event.name} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                                <div className="flex items-start gap-3 relative cursor-pointer hover:bg-muted/30 rounded-lg p-2 transition-colors" onClick={() => setExpandedEvent(isExpanded ? null : event.name)}>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-muted-foreground mt-0.5">{t("systemEvents.by", "By")}: {event.triggered_by || t("systemEvents.system", "System")}</p>
            ...
            {!isLoading && filtered.length === 0 && (
                <div className="text-center py-16"><Activity className="w-16 h-16 text-muted-foreground/20 mx-auto mb-4" /><p className="text-muted-foreground">{t("systemEvents.noEvents", "No events found")}</p></div>
            )}
        </motion.div>
    );
}
